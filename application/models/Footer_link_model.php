...
class Footer_link_model extends CI_Model {
    ...
    function _Count_All($status = '') {
        $this->db->select('id')->from('footer_link');
        if($status) {
    ...
    function _Get_By_Id($id) {
        return $this->db->where('id', $id)->get('footer_link')->row_array();
    }
    
    function _Add($data) {
        $this->db->insert('footer_link', array(
    ...
    function _Update($data, $id) {
        $this->db->where('id', $id)->update('footer_link', array(
            'title' => trim($data['title']),
            'link' => $data['link'],
            'parent_id' => $data['parent_id'],
            'category_id' => intval($data['category_id']),
    ...
    function _Update_Status($status, $id) {
     ...
     function _Update_Sort($id, $value) {
        $this->db->where('id', intval($id))->update('footer_link', array('sort_order'=>$value));
    }
    
    function _Delete($id) {
        $category = $this->_Get_By_Id($id);
        if($category) {
            $this->db->where('id', $id)->delete('footer_link');
