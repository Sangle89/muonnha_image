...
class Popup_model extends CI_Model {
    ...
    function _Get_All($per_page=9999, $page=0, $status='') {
        ...
        $result = $this->db->get('popups')->result_array();
        
        return $result;
    }
    
    function _Get_By_Id($id) {
        return $this->db->where('id', $id)->get('popups')->row_array();
    }
    
    function _Add($data) {
        
        $this->db->insert('popups', array(
    ...
    function _Update_Status($status, $id) {
        $this->db->where('id', $id)->update('popups', array(
            'status' => trim($status)
        ));
    }
    
     function _Update_Sort($id, $value) {
        $this->db->where('id', intval($id))->update('popups', array('sort_order'=>$value));
    }
    
    function _Delete($id) {
        $category = $this->_Get_By_Id($id);
        if($category) {
            $this->db->where('id', $id)->delete('popups');
