...
class Ion_auth
{
      ...
      $user = $this->where($identifier, $identity)->where('active', 1)->users()->row();  // changed to get_user_by_identity from email
			if ($user)
			{
				...
				if(!$this->config->item('use_ci_email', 'ion_auth'))
				{
					...
					get_instance()->load->library('send_email');
                    $send = get_instance()->send_email->_Send('Khôi phục mật khẩu', $message, 'admin@admin.com',$user->email);
                    if ($send)
					{
						$this->set_message('forgot_password_successful');
						return TRUE;
	...
	public function register($identity, $password, $email, $additional_data = array(), $group_ids = array()) //need to test email activation
	{
		...
		else
		{
			...
			else
			{
                ...
                if($this->send_email->_Send($subject, $message, $from, $email)){
                    $this->ion_auth_model->trigger_events(array('post_account_creation', 'post_account_creation_successful', 'activation_email_successful'));
					$this->set_message('activation_email_successful');
					return $id;
                }
    ...
    public function is_writer($id=false)
	{
		$this->ion_auth_model->trigger_events('is_writer');
		$admin_group = $this->config->item('writer_group', 'ion_auth');
		return $this->in_group($admin_group, $id);
	}
	/**
	 * in_group
